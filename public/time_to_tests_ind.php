...
<?php
  ...
  //end of test
  $total_count = selected_stud_Test::count_all($current_subject);
  ...
  $sql  = "SELECT * FROM test_for_selected_stud ";
...
<?php
  ...
  switch(true){  
    case($check == 1 && !empty($test->truth1)) :
      $session->increase();
      $session->num_page();
      redirect_to("time_to_tests_ind.php?page={$pagination->next_page()}&subject={$test->subject_id}"); 
